...
export default function Home({ isAuth }: { isAuth: boolean }) {
  ...
  const searchByCleaner = () => {
    api
      .get(`/cleaner/getCleanerByNameOrUsername/${searchValue}`)
      .then((response) => {
  ...
  const searchByCity = () => {
    api
      .get(`/cleaner/getCleanerByCity/${searchValueCity}`)
      .then((response) => {
        setArrayCleaner(response.data);
      })
      .catch(() => {
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ color: "white", fontWeight: "bold", fontSize: 45 }}>
          Find an appointment with our cleaning staff
        </p>
        <div
          style={{
            display: "flex",
      ...
      >
        {arrayCleaner.length !== 0 &&
          arrayCleaner.map((cleaner, index) => {
            return (
