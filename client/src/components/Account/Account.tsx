...
export default function Account({
  ...
  const fetchAvailable = async () => {
    ...
    const res = await api.get(
      `/cleaner/available/${userInfo.id}/${year}/${month}`
    );
    ...
    fullMonthArray.forEach((value: Date) => {
      ...
      data.forEach((date: any) => {
        //any car c'est le retour de l'api faudrait le fix
  ...
  const fetchProposal = () => {
    console.log("api");
    const stringApi =
      isCleaner === 1 ? "getAllProposals" : "getClientProposals";
    api.get("/cleaner/" + stringApi).then((response) => {
              ...
              return (
                <div
                  ...
                  style={{
                    padding: 10,
                    backgroundColor:
                      available.isSelected === false ? "red" : "#0D6EFD",
                    margin: 5,
                    width: 100,
                    alignItems: "center",
                    height: 50,
